#PBS -N moco_lincl_mlp_RN50_cos_01_loss
#PBS -S /bin/bash
...
python3 /misc/student/alzouabk/Thesis/self_supervised_pretraining/moco/main_lincls.py \
  --exp 'moco_lincl_mlp_RN50_cos_01_loss' \
  --epochs 40 \
  --print-freq 1 \
  --optimizer adam \
  --arch resnet50 \
  --lr 0.1 \
  --batch-size 48 \
  --pretrained /misc/student/alzouabk/Thesis/self_supervised_pretraining/moco/outputs_2/mlp_RN50_cos_01/best_model_loss.pth.tar \
